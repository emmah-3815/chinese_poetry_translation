...
def main():
    ...
    # Peak VRAM recorded by PyTorch allocator during training
    vram_bytes    = torch.cuda.max_memory_reserved() if torch.cuda.is_available() else 0
    vram_gb       = vram_bytes / 1024 ** 3

    model.save_pretrained(str(out))
    tokenizer.save_pretrained(str(out))
    print(f"Training on : {args.model_name}")
    print(f"\n  Training time : {hours:02d}h {minutes:02d}m {seconds:05.2f}s  "
          f"({elapsed:.1f}s total)")
    print(f"  Per epoch     : {per_epoch:.1f}s  ({per_epoch/60:.1f} min)")
    print(f"  Peak VRAM     : {vram_gb:.2f} GB")

    # Save timing + VRAM to a small JSON for easy comparison across experiments
